  ...
  try {
    const result = await pool.query(
      `UPDATE content
       ...
       RETURNING id, title, ai_score`
    );
    res.json({ cleaned: result.rowCount, rejected: result.rows });
  } catch (error) {
